...
export default function handler(req, res){
  ...
  res.setHeader("Access-Control-Allow-Origin", "https://ai-4-chat-ai-assistant.vercel.app");
...
async function handleMessage(message, session) {
    if (isPersonalizedQuery(message)) {
        return {
            response: handlePersonalizedQuery(message),
            sources: []
        };
    }
    ...
    try {
        const rawResults = await searchKnowledgeBase(message, 5);
        const normResults = Array.isArray(rawResults) ? rawResults : [];
        const response = await generateResponse(message, normResults, session);
        ...
        console.error('Search/Response generation failed:', error);
        throw error;
    }
}
